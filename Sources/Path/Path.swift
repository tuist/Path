...
public struct AbsolutePath: Hashable, Sendable {
    ...
    static func isValidComponent(_ name: String) -> Bool {
        PathImpl.isValidComponent(name)
    }
    ...
    public var parentDirectory: AbsolutePath {
        AbsolutePath(_impl.parentDirectory)
    }
    ...
    public func appending(component: String) -> AbsolutePath {
        AbsolutePath(_impl.appending(component: component))
    }
    ...
    public func appending(components names: [String]) -> AbsolutePath {
        ...
        names.reduce(self) { path, name in
    ...
    public func prettyPath(cwd: AbsolutePath?) -> String {
...
public struct RelativePath: Hashable, Sendable {
    ...
    public func appending(_ subpath: RelativePath) -> RelativePath {
        RelativePath(_impl.appending(relativePath: subpath._impl))
    }
    ...
    public func appending(component: String) -> RelativePath {
        RelativePath(_impl.appending(component: component))
    }
...
extension Path {
    ...
    private struct WindowsPath: Path, Sendable {
        ...
        static func isValidComponent(_ name: String) -> Bool {
            name != "" && name != "." && name != ".." && !name.contains("/")
        }

        static func isAbsolutePath(_ path: String) -> Bool {
            !path.withCString(encodedAs: UTF16.self, PathIsRelativeW)
        }
        ...
        var isAbsolute: Bool {
            Self.isAbsolutePath(string)
        }

        public var isRoot: Bool {
            string.withCString(encodedAs: UTF16.self, PathCchIsRoot)
        }
        ...
        func suffix(withDot: Bool) -> String? {
            string.withCString(encodedAs: UTF16.self) {
...
extension PathValidationError: CustomStringConvertible {
    public var description: String {
        ...
        case let .startsWithTilde(path):
            "invalid absolute path '\(path)'; absolute path must begin with '/'"
        case let .invalidAbsolutePath(path):
            "invalid absolute path '\(path)'"
        case let .invalidRelativePath(path):
            "invalid relative path '\(path)'; relative path should not begin with '\(AbsolutePath.root.pathString)'"
        }
    }
}
...
extension AbsolutePath {
    ...
    public func contains(_ other: AbsolutePath) -> Bool {
        isDescendantOfOrEqual(to: other)
    }
    ...
    public func isAncestor(of descendant: AbsolutePath) -> Bool {
        descendant.components.dropLast().starts(with: components)
    }
    ...
    public func isAncestorOfOrEqual(to descendant: AbsolutePath) -> Bool {
        descendant.components.starts(with: components)
    }
    ...
    public func isDescendant(of ancestor: AbsolutePath) -> Bool {
        components.dropLast().starts(with: ancestor.components)
    }
